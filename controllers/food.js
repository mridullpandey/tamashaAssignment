...
exports.takenNewFood = (req, res, next) => {
  ...
  if (!req.body.taken_date_time) {
    return res
      .status(400)
      .send({ error: "Food taken date and time is required!!" });
  }
  ...
  Foods.getTodayTakenFoodsByUserId(req.body.userId)
    .then(async (result) => {
      if (result && result.length > 0) {
        let totalCalories = 0;
        for (let i = 0; i < result.length; i++) {
          totalCalories = totalCalories + result[i]["calorie_value"];
        }
        ...
        Foods.placeNewFood(req.body)
          .then((result) => {
            res.status(200).json({
              status: 200,
              message: "Food successfully Placed..",
              data: result,
            });
          })
          .catch((error) => {
      ...
      if (user && user.length > 0 && user["userRole"] == "admin") {
        Foods.getFoodList()
          .then((resp) => {
            res
              .status(200)
              .json({ status: 200, message: "SUCCESS", data: resp });
          })
          .catch((error) => {
            res.status(500).json({ status: 500, error: error });
          });
      } else {
        Foods.getFoodsByUserId(req.params.userId)
          .then((result) => {
            res
              .status(200)
...
exports.getLastXDaysEntry = (req, res, next) => {
  if (req.params.days) {
    Foods.getLastXDayEntry(req.params.days)
      .then((result) => {
...
exports.getUsersAverageCalories = (req, res, next) => {
  if (req.params.days) {
    Foods.getUsersAVGCalaries(req.params.days)
      .then((result) => {
        res.status(200).json({ status: 200, message: "SUCCESS", data: result });
      })
      .catch((error) => {
